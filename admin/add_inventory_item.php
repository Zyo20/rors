...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // If no errors, insert the inventory item
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO inventory (name, description, quantity, unit, threshold, updated_at) VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssdsd", $name, $description, $quantity, $unit, $threshold);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Error adding inventory item: " . $conn->error;
        }
...
<head>
    ...
    <title>Add Inventory Item - Restaurant Ordering and Reservation System</title>
...
<body class="bg-gray-100 min-h-screen flex flex-col">
    ...
    <main class="flex-grow container mx-auto px-4 py-8">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold mt-2">Add New Inventory Item</h1>
        </div>
            ...
            <form action="" method="post">
                ...
                <div class="mt-6 flex justify-end">
                    ...
                    <button type="submit" class="bg-primary text-white px-4 py-2 rounded-md hover:bg-primary-dark transition">
                        <i class="fas fa-save mr-2"></i> Save Inventory Item
                    </button>
                </div>
            </form>
        </div>
